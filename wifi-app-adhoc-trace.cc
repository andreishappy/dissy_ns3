...
int DEFAULT_SEND_P = 60000; //in milliseconds
...
public:
	...
	void UpdateShortestPath() {
    	...
    	ss << "Time: " << Simulator::Now().GetSeconds() << " Node: " << m_nodeId << " Neighbours: \n";

        for (it = entry.begin(); it!=entry.end(); it++) {
    	   	if (AddressToNodeId.find(it->destAddr)->second == producer) {
    	   		ss << " Distance to " << producer << " "<< it->distance <<"\n";
    	   		Log(ss);
    	   		shortestPathLength = it->distance;
    	   	}
    	}
	...
	void Reconfigure() {
		if (shortestPathLength != 0)
		{
			...
			int ownDistanceToProducer = FindDistanceToNode(producer);
		    int ownDistanceToConsumer = FindDistanceToNode(consumer);

		    // Producer Frequency
		    if (m_nodeId == producer) {
		    	if( ownDistanceToConsumer != 0) {
		    		Time new_m_synchFrequency = MilliSeconds(REQUIREMENT.GetMilliSeconds() / SPEED_UP / shortestPathLength);
		    		SetNextSendTimer(new_m_synchFrequency);
		    		m_consumerToProducerDistance = shortestPathLength;
		    	} else {
		    	    SetNextSendTimer(MilliSeconds(DEFAULT_SEND_P));
		    	}
		    }
		    ...
		    if (m_nodeId != consumer) {
		    	if ( ownDistanceToConsumer != 0 && ownDistanceToProducer != 0)
		        {
		    	    ...
		    	    m_zoneValue = (float)m_consumerToProducerDistance / shortestPathLength;

		    	    if ( ownDistanceToConsumer + ownDistanceToProducer <= shortestPathLength + ZONE_THRESHOLD )
		    	    {
		    	    	Time new_m_synchFrequency = MilliSeconds(REQUIREMENT.GetMilliSeconds() / SPEED_UP / shortestPathLength);
		    	    	SetNextSendTimer(new_m_synchFrequency);
		    	    } else {
		    	    	SetNextSendTimer(DEFAULT_FREQUENCY);
		    	    }
		        }
		        else
		        {
		...
		m_shortestPathLength = shortestPathLength;
